...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    data[uid]["money"] += 금액
    save_data(data)
    embed = discord.Embed(title="✨ 관리자 지급 완료", color=0xffd700)
    embed.add_field(name="받은 금액", value=f"+{금액:,}원", inline=True)
    embed.add_field(name="💰 현재 잔액", value=f"**{data[uid]['money']:,}원**", inline=True)
...
async def check_wallet(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="👛 내 지갑 정보", color=0xff69b4)
    embed.add_field(name="💰 보유 금액", value=f"**{u['money']:,}원**", inline=False)
    embed.add_field(name="📊 승률 정보", value=f"{u['wins']}승 {u['losses']}패 (승률: {win_rate:.1f}%)", inline=False)
    await interaction.followup.send(embed=embed)
...
async def daily(interaction: discord.Interaction):
    ...
    if data[uid].get("last_daily") == today:
        embed = discord.Embed(title="❌ 출석 실패", description="이미 오늘의 출석을 완료하셨어요!", color=0xff4500)
        return await interaction.followup.send(embed=embed)
    data[uid]["money"] += 10000
    data[uid]["last_daily"] = today
    save_data(data)
    embed = discord.Embed(title="✅ 출석 체크!", color=0x87ceeb)
...
async def reward(interaction: discord.Interaction):
    ...
    if last and datetime.now() < datetime.strptime(last, "%Y-%m-%d %H:%M:%S") + timedelta(hours=3):
        embed = discord.Embed(title="⏱️ 대기 시간", description="3시간 후에 다시 받아주세요!", color=0xffd700)
        return await interaction.followup.send(embed=embed)
    data[uid]["money"] += 50000
    data[uid]["last_reward"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    save_data(data)
    embed = discord.Embed(title="🎁 보상금 수령", color=0x98fb98)
...
async def loan(interaction: discord.Interaction):
    ...
    if data[uid]["money"] > -500000:
        embed = discord.Embed(title="❌ 대출 불가", description="잔액이 -500,000원 이하여야 합니다.", color=0xff4500)
        return await interaction.followup.send(embed=embed)
    now = datetime.now()
    last = data[uid].get("last_loan", "")
    if last and now < datetime.strptime(last, "%Y-%m-%d %H:%M:%S") + timedelta(hours=24):
        embed = discord.Embed(title="⏱️ 쿨타임", description="24시간에 한 번만 대출 가능해요!", color=0xffd700)
        return await interaction.followup.send(embed=embed)
    data[uid]["money"] = 200000
    data[uid]["last_loan"] = now.strftime("%Y-%m-%d %H:%M:%S")
    save_data(data)
    embed = discord.Embed(title="✅ 대출 승인", color=0x32cd32)
...
async def support(interaction: discord.Interaction):
    ...
    if data[uid]["money"] > -10000:
        embed = discord.Embed(title="❌ 지원 불가", description="잔액이 -10,000원 이하여야 합니다.", color=0xff4500)
        return await interaction.followup.send(embed=embed)
    now = datetime.now()
    last = data[uid].get("last_support", "")
    if last and now < datetime.strptime(last, "%Y-%m-%d %H:%M:%S") + timedelta(hours=12):
        embed = discord.Embed(title="⏱️ 쿨타임", description="12시간에 한 번만 받을 수 있어요!", color=0xffd700)
        return await interaction.followup.send(embed=embed)
    amount = random.randint(100000, 400000)
    data[uid]["money"] += amount
    data[uid]["last_support"] = now.strftime("%Y-%m-%d %H:%M:%S")
    save_data(data)
    embed = discord.Embed(title="🎁 재난지원금", color=0xdda0dd)
    embed.add_field(name="지원 금액", value=f"+{amount:,}원", inline=True)
    embed.add_field(name="💰 현재 잔액", value=f"**{data[uid]['money']:,}원**", inline=True)
    await interaction.followup.send(embed=embed)
