...
def init_user(data, user_id):
    ...
    # 기존 데이터 유저들의 마이그레이션을 위해 키 확인 보강
...
# 🏥 /재난지원금 (12시간마다 10만 ~ 40만 랜덤 지급 명령어 추가)
@bot.tree.command(name="재난지원금", description="12시간마다 한 번씩 100,000 ~ 400,000머니를 랜덤으로 받습니다.")
async def emergency_money(interaction: discord.Interaction):
    ...
    if last_emergency_str:
        try:
            ...
            next_emergency_time = last_emergency_time + timedelta(hours=12)
            if now < next_emergency_time:
                ...
                embed.description = f"⏱️ **재난지원금은 12시간마다 한 번씩만 신청할 수 있습니다!**\n\n> ⏳ **대기 시간 : {hours}시간 {minutes}분 {seconds}초 후 가능**\n\n🔹 **잔액 : {data[user_id]['money']:,}머니**"
                await interaction.followup.send(embed=embed)
                return
        except:
            pass
            
    # 100,000머니 ~ 400,000머니 사이의 금액 무작위 연산
...
class GambleView(discord.ui.View):
    def __init__(self, user_id, 배팅액, win_multiplier, lose_multiplier, 모드이름):
        ...
        self.모드이름 = 모드이름
        self.random_probability = random.randint(1, 100) # 1~100 사이 랜덤 승률 결정
    ...
    async def check_result(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # 승리 조건 연산 (원본 message.txt 고정 배율 로직 완벽 유지)
...
async def gamble(interaction: discord.Interaction, 배팅액: int, 레버러지: app_commands.Choice[int]):
    ...
    # 마이너스 잔액 검사 문구에 '재난지원금' 단어 추가 복구 완료
    ...
    # 대기 상태 임베드 세팅 (image_2bb780.png 스타일 완벽 반영)
