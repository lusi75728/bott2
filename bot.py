...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    embed.description = f"👤 **대상:** {유저.mention}\n💰 **지급액:** +{금액:,}머니\n🔹 **최종 잔액:** {data[str(유저.id)]['money']:,}머니"
...
async def daily(interaction: discord.Interaction):
    ...
    embed.description = f"💰 **+10,000머니 지급!**\n🔹 **현재 잔액 :** {data[user_id]['money']:,}머니"
    await interaction.response.send_message(embed=embed)
...
async def reward(interaction: discord.Interaction):
    ...
    embed.description = f"💰 **+30,000머니 지급!**\n🔹 **현재 잔액 :** {data[user_id]['money']:,}머니"
    await interaction.response.send_message(embed=embed)
...
class GambleView(discord.ui.View):
    ...
    async def check_result(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if win: data[self.user_id]["money"] += self.배팅액; data[self.user_id]["wins"] += 1; title, color = "🎉 도박에 성공했어요!", 0x3498db
        else: data[self.user_id]["money"] -= self.배팅액; data[self.user_id]["losses"] += 1; title, color = "💥 도박에 실패했어요", 0xe74c3c
        save_data(data)
        embed = discord.Embed(title=title, color=color)
        embed.description = f"🎯 **결과 :** {self.배팅액:,}머니 {'획득' if win else '손실'}\n🔹 **현재 잔액 :** {data[self.user_id]['money']:,}머니"
