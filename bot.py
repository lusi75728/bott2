...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    embed.add_field(name="지급액", value=f"+{금액:,} 원", inline=True)
    embed.add_field(name="최종 잔액", value=f"{data[str(유저.id)]['money']:,} 원", inline=False)
...
async def check_wallet(interaction: discord.Interaction):
    ...
    embed.add_field(name="💰 보유 금액", value=f"{u['money']:,} 원", inline=True)
    embed.add_field(name="📊 도박 전적", value=f"{u['wins']}승 {u['losses']}패", inline=True)
    await interaction.response.send_message(embed=embed)
...
async def daily(interaction: discord.Interaction):
    ...
    if data[user_id]["last_daily"] == today:
        return await interaction.response.send_message("🚨 이미 오늘 출석하셨습니다.", ephemeral=True)
    data[user_id]["money"] += 10000; data[user_id]["last_daily"] = today; save_data(data)
    embed = discord.Embed(title="💵 출석체크 완료", color=0x2ecc71)
    embed.add_field(name="보상", value="+10,000 원", inline=True)
    embed.add_field(name="현재 잔액", value=f"{data[user_id]['money']:,} 원", inline=True)
    await interaction.response.send_message(embed=embed)
...
async def reward(interaction: discord.Interaction):
    ...
    embed.add_field(name="보상", value="+30,000 원", inline=True)
    embed.add_field(name="현재 잔액", value=f"{data[user_id]['money']:,} 원", inline=True)
    await interaction.response.send_message(embed=embed)
...
async def emergency_money(interaction: discord.Interaction):
    ...
    embed.add_field(name="획득 금액", value=f"+{amt:,} 원", inline=True)
    embed.add_field(name="현재 잔액", value=f"{data[user_id]['money']:,} 원", inline=True)
    await interaction.response.send_message(embed=embed)
...
async def send_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    embed.add_field(name="송금 금액", value=f"{금액:,} 원", inline=True)
    embed.add_field(name="나의 잔액", value=f"{data[f_id]['money']:,} 원", inline=False)
...
class GambleView(discord.ui.View):
    ...
    async def check_result(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="결과", value=f"{self.배팅액:,} 원 {'획득' if win else '손실'}", inline=True)
        embed.add_field(name="현재 잔액", value=f"{data[self.user_id]['money']:,} 원", inline=True)
...
async def leader_board(interaction: discord.Interaction):
    ...
    desc = "\n".join([f"**{i+1}위** : <@{uid}> - **{m:,} 원**" for i, (uid, m) in enumerate(rank)])
