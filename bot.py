...
# 유저 데이터 초기화 및 마이너스 잔액 연산 정수(int) 보장 함수
...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    current_money = int(data[target_id].get("money", 0))
    data[target_id]["money"] = current_money + 금액
    save_data(data)
...
async def check_wallet(interaction: discord.Interaction):
    ...
    money = int(data[user_id].get("money", 0))
...
async def daily(interaction: discord.Interaction):
    ...
    if data[user_id].get("last_daily") == today_str:
        embed = discord.Embed(title="🚨 이미 오늘 돈을 받으셨습니다", description="출석체크", color=0xe74c3c)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
        
    await interaction.response.defer()
    current_money = int(data[user_id].get("money", 0))
    data[user_id]["money"] = current_money + 10000
    data[user_id]["last_daily"] = today_str
    save_data(data)
    
    embed = discord.Embed(title="💵 출석체크 완료", color=0x2ecc71)
    embed.description = f"👤 **{interaction.user.mention}**\n\n> 💰 **지급된 금액 : +10,000머니**\n\n🔹 **현재 잔액 : {data[user_id]['money']:,}머니**"
...
async def reward(interaction: discord.Interaction):
    ...
    if last_reward_str:
        try:
            ...
            if now < next_reward_time:
                ...
                embed = discord.Embed(title="🚨 이미 오늘 돈을 받으셨습니다", description="보상금", color=0xe74c3c)
                embed.set_footer(text=f"⏱️ 대기 시간: {hours}시간 {minutes}분 {seconds}초 후 가능")
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
        except ValueError:
    ...
    await interaction.response.defer()
    current_money = int(data[user_id].get("money", 0))
    data[user_id]["money"] = current_money + 30000
    ...
    if last_emergency_str:
        try:
            ...
            if now < next_emergency_time:
                ...
                embed = discord.Embed(title="🚨 이미 오늘 돈을 받으셨습니다", description="재난지원금", color=0xe74c3c)
                embed.set_footer(text=f"⏱️ 대기 시간: {hours}시간 {minutes}분 {seconds}초 후 가능")
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
        except ValueError:
...
async def send_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    from_money = int(data[from_id].get("money", 0))
    if from_money < 금액:
        embed = discord.Embed(title="❌ 송금 실패", description=f"보유 잔액이 부족합니다.\n현재 보유량: **{from_money:,}머니**", color=0xe74c3c)
        await interaction.followup.send(embed=embed, ephemeral=True)
        return
...
class GambleView(discord.ui.View):
    ...
    async def check_result(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        current_money = int(data[self.user_id].get("money", 0))
        dice = random.randint(1, 100)
        
        # 승리 조건 연산
        if dice <= self.random_probability:
            net_profit = self.배팅액 * self.win_multiplier
            data[self.user_id]["money"] = current_money + net_profit
        ...
        # 패배 조건 연산
            ...
            data[self.user_id]["money"] = current_money - net_loss
...
async def gamble(interaction: discord.Interaction, 배팅액: int, 레버러지: app_commands.Choice[int]):
    ...
    if current_money < 0:
        embed = discord.Embed(
            title="❌ 도박 제한", 
            description=f"현재 잔액이 마이너스(**{current_money:,}머니**) 상태입니다.\n재난지원금이나 보상금으로 돈을 모아 양수(+)를 만든 후 다시 시도해 주세요!", 
            color=0xe74c3c
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    if current_money < 배팅액:
        embed = discord.Embed(
            title="❌ 배팅 실패",
            description=f"**배팅 금액이 보유 잔액보다 많습니다.**\n\n> 💸 **배팅 시도액 : {배팅액:,}머니**\n\n🔹 **현재 잔액 : {current_money:,}머니**",
            color=0xe74c3c
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
...
async def leader_board(interaction: discord.Interaction):
    ...
    for user_id, user_info in data.items():
        if isinstance(user_info, dict):
            try:
                money_val = int(user_info.get("money", 0))
            except:
                money_val = 0
            ranking.append((user_id, money_val))
