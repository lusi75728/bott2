...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    save_data(data)
    embed = discord.Embed(title="👑 관리자 지급 완료", description=f"대상: {유저.mention}\n지급액: **+{금액:,}머니**", color=0xf1c40f)
...
async def check_wallet(interaction: discord.Interaction):
    ...
    embed.description = f"💰 **보유 금액 : {u['money']:,}머니**\n📊 **도박 전적 : {u['wins']}승 {u['losses']}패**"
    await interaction.response.send_message(embed=embed)
...
async def daily(interaction: discord.Interaction):
    ...
    if data[user_id]["last_daily"] == today: return await interaction.response.send_message("🚨 이미 오늘 출석했습니다.", ephemeral=True)
    data[user_id]["money"] += 10000; data[user_id]["last_daily"] = today; save_data(data)
    embed = discord.Embed(title="💵 출석체크 완료", description=f"💰 **+10,000머니 지급**\n🔹 **현재 잔액 : {data[user_id]['money']:,}머니**", color=0x2ecc71)
    await interaction.response.send_message(embed=embed)
...
async def reward(interaction: discord.Interaction):
    ...
    if last and datetime.strptime(last, "%Y-%m-%d %H:%M:%S") + timedelta(hours=3) > now: return await interaction.response.send_message("🚨 대기 시간이 남았습니다.", ephemeral=True)
    data[user_id]["money"] += 30000; data[user_id]["last_reward"] = now.strftime("%Y-%m-%d %H:%M:%S"); save_data(data)
    embed = discord.Embed(title="🎁 보상금 수령", description=f"💰 **+30,000머니 지급**\n🔹 **현재 잔액 : {data[user_id]['money']:,}머니**", color=0x2ecc71)
    await interaction.response.send_message(embed=embed)
...
async def emergency_money(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🏥 재난지원금", description=f"💰 **+{amt:,}머니 획득!**\n🔹 **현재 잔액 : {data[user_id]['money']:,}머니**", color=0x3498db)
    await interaction.response.send_message(embed=embed)
...
class GambleView(discord.ui.View):
    ...
    async def check_result(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        save_data(data)
        embed = discord.Embed(title=title, description=f"🎯 **결과 : {self.배팅액:,}머니 {'획득' if win else '손실'}**\n🔹 **현재 잔액 : {data[self.user_id]['money']:,}머니**", color=color)
...
async def gamble(interaction: discord.Interaction, 배팅액: int):
    data = load_data(); init_user(data, str(interaction.user.id))
    if data[str(interaction.user.id)]["money"] < 배팅액: return await interaction.response.send_message("❌ 잔액이 부족합니다.", ephemeral=True)
...
async def leader_board(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=discord.Embed(title="🏆 서버 랭킹 (Top 10)", description=desc, color=0xf1c40f))
