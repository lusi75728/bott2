...
def init_user(data, user_id):
    ...
    # 🔹 데이터가 문자열로 꼬여있는 경우를 대비해 숫자로 강제 치환
    try:
...
async def add_money(interaction: discord.Interaction, 유저: discord.Member, 금액: int):
    ...
    # 🔹 마이너스 상태에서도 정상 연산되도록 int 변환 후 가산
...
async def daily(interaction: discord.Interaction):
    ...
    # 🔹 마이너스 잔액에서 차감(합산)되도록 int 강제 변환 연산
    current_money = int(data[user_id].get("money", 0))
...
# 3. /보상금 (3시간 마다 30,000머니 - 쿨타임 완벽 보장)
...
# 🏥 /재난지원금 (3시간 마다 10만 ~ 40만 랜덤 지급 - 마이너스 차감 정상화)
